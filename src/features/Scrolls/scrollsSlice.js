...
const scrollsSlice = createSlice({
  ...
  reducers: {
    addScroll: (state, action) => {
      // payload = new scroll
      state.scrolls.push(action.payload);
    },

    deleteScroll: (state, action) => {
      // payload = id
      state.scrolls = state.scrolls.filter(
        (scroll) => Number(scroll.id) !== Number(action.payload),
      );
    },
    ...
    updateScroll: (state, action) => {
      ...
      const scrollToUpdate = state.scrolls.find(
        (scroll) => Number(scroll.id) === Number(id),
      );
      if (scrollToUpdate) {
        scrollToUpdate.title = newScroll.title;
        scrollToUpdate.tome = newScroll.tome; // Category
        scrollToUpdate.content = newScroll.content;
      }
    },
  },
});
